...
def _extract_budget(message: str):
    # Prefer values explicitly marked as money/budget to avoid mixing with guest count.
    budget_patterns = [
        r"(?:\$|usd\s*)(\d+(?:\.\d+)?)",
        r"(\d+(?:\.\d+)?)\s*\$",
    ...
    # Fallback: choose the largest number in message (usually budget > client count).
...
def assistant_chat(request):
    ...
    try:
        ...
        grounded_prompt = (
            "You are a hotel assistant. Use the provided database context as source of truth for availability and prices. "
            "If user asks for suggestions, prioritize matching rooms by budget and client count from the database. "
            "If there is no exact match, suggest closest alternatives and explain why. "
